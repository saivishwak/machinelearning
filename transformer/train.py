...
# TODO: Do better we are lossing lot of info here
def truncate_or_pad(text, max_len):
    """Truncate or pad the text to ensure it has a maximum length of max_len."""
    if len(text) > max_len:
        return text[:max_len]  # Truncate if longer
    else:
        return text
...
def create_translation_dataset(english_file, hindi_file, max_seq_len=500):
    ...
    data = [
        {
            'en': truncate_or_pad(eng.strip(), max_seq_len),
            'hi': truncate_or_pad(hin.strip(), max_seq_len)
        } 
...
#     # Create a list of dictionaries where each dictionary contains 'english' and 'hindi' texts
#     data = [{'en': eng.strip(), 'hi': hin.strip()} 
#             for eng, hin in zip(english_texts, hindi_texts)]
...
def get_indian_ds(config):
    # It only has the train split, so we divide it overselves
    # ds_raw = load_dataset('opus_books', f"{config['lang_src']}-{config['lang_tgt']}", split='train')
    ds_raw = create_translation_dataset(config['src_lang_file'], config['tgt_lang_file'])
    ...
    print('Loaded Indian Language dataset')
...
def train_model(config):
    ...
    train_dataloader, val_dataloader, tokenizer_src, tokenizer_tgt = get_indian_ds(config)
